...
{
    class RemoveCollectionSvc
    {

        public static async Task execute(string credential, string url, string username)
        {
            // Generate module header
            Console.WriteLine(Utilities.ArgUtils.GenerateHeader("removecollectionsvc", credential, url, "", "", "", username));
            ...
            if (await Utilities.WebUtils.credsValid(credential, url))
            {
                ...
                try
                {
                    ...
                    // if user was removed successfully, display message and list the members of the project collection service accounts group
                    if (userRemoved)
                    {
