...
{
    class AddCollectionSvc
    {

        public static async Task execute(string credential, string url, string username)
        {
            // Generate module header
            Console.WriteLine(Utilities.ArgUtils.GenerateHeader("addcollectionsvc", credential, url, "", "", "", username));
                ...
                try
                {
                    ...
                    foreach (Objects.User user in userList)
                    {
                        ...
                        if (user.directoryAlias.ToLower().Equals(username.ToLower()))
                        {
                            ...
                            // iterate through the list of groups and grab the desriptor for the collection service accounts group
                            foreach (Objects.Group group in groupList)
                            {
                                if (group.displayName.ToLower().Equals("project collection service accounts"))
                                {
                    ...
                    Console.WriteLine("");
                    Console.WriteLine("[*] INFO: Attempting to add " + username + " to the Project Collection Service Accounts group.");
                    Console.WriteLine("");
                    ...
                    // if user was added successfully, display message and list the members of the project collection service accounts group
                    if (userAdded)
                    {
                        ...
                        // iterate through the list of groups for the project and list the members of the project collection service accounts group
                        foreach (Objects.Group group in groupList)
                        {
                            // if the group is project collection service accounts
                            if (group.displayName.ToLower().Equals("project collection service accounts"))
                            {
