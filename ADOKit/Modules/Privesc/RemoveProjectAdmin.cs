...
{
    class RemoveProjectAdmin
    {

        public static async Task execute(string credential, string url, string projectName, string username)
        {
            // Generate module header
            Console.WriteLine(Utilities.ArgUtils.GenerateHeader("removeprojectadmin", credential, url, "", projectName, "", username));
                ...
                try
                {
                    ...
                    foreach (Objects.User user in userList)
                    {
                        ...
                        if (user.directoryAlias.ToLower().Equals(username.ToLower()))
                        {
                            ...
                            // get a listing of groups for the project we are wanting to add our user to as a project admin
                            List<Objects.Group> groupList = await Utilities.GroupUtils.getGroupPermissionsForProject(credential, url, projectName);

                            // iterate through the list of groups for the project and get the descriptor for the project administrators group
                            foreach (Objects.Group group in groupList)
                            {
                                if (group.displayName.ToLower().Equals("project administrators"))
                                {
                    ...
                    // if user was removed successfully, display message and list the members of the project admin group
                    if (userRemoved)
                    {
                        ...
                        // iterate through the list of groups for the project and list the members of the project administrators group
                        foreach (Objects.Group group in groupList)
                        {
