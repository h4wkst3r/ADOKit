...
{
    class RemoveCollectionBuildAdmin
    {

        public static async Task execute(string credential, string url, string username)
        {
            // Generate module header
            Console.WriteLine(Utilities.ArgUtils.GenerateHeader("removecollectionbuildadmin", credential, url, "", "", "", username));
                ...
                try
                {
                    ...
                    foreach (Objects.User user in userList)
                    {
                        ...
                        if (user.directoryAlias.ToLower().Equals(username.ToLower()))
                        {
                            ...
                            // iterate through the list of groups and grab the desriptor for the collection build admin group
                            foreach (Objects.Group group in groupList)
                            {
                                if (group.displayName.ToLower().Equals("project collection build administrators"))
                                {
                    ...
                    // if user was removed successfully, display message and list the members of the project collection build admin group
                    if (userRemoved)
                    {
                        ...
                        // iterate through the list of groups for the project and list the members of the project collection build administrators group
                        foreach (Objects.Group group in groupList)
                        {
                            // if the group is project collection build administrators
                            if (group.displayName.ToLower().Equals("project collection build administrators"))
                            {
