...
{
    class RemoveBuildAdmin
    {

        public static async Task execute(string credential, string url, string projectName, string username)
        {
            // Generate module header
            Console.WriteLine(Utilities.ArgUtils.GenerateHeader("removebuildadmin", credential, url, "", projectName, "", username));
                ...
                try
                {
                    ...
                    foreach (Objects.User user in userList)
                    {
                        ...
                        if (user.directoryAlias.ToLower().Equals(username.ToLower()))
                        {
                            ...
                            // get a listing of groups for the project we are wanting to remove build admin from
                    ...
                    // if user was removed successfully, display message and list the members of the build admin group
                    if (userRemoved)
                    {
                        ...
                        foreach (Objects.Group group in groupList)
                        {
                            // if the group is build administrators
                            if (group.displayName.ToLower().Equals("build administrators"))
                            {
