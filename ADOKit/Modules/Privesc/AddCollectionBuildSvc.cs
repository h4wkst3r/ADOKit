...
{
    class AddCollectionBuildSvc
    {

        public static async Task execute(string credential, string url, string username)
        {
            // Generate module header
            Console.WriteLine(Utilities.ArgUtils.GenerateHeader("addcollectionbuildsvc", credential, url, "", "", "", username));
            ...
            if (await Utilities.WebUtils.credsValid(credential, url))
            {
                ...
                try
                {
                    ...
                    // if user was added successfully, display message and list the members of the project collection build service accounts group
                    if (userAdded)
                    {
