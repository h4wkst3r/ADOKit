...
{
    class AddProjectAdmin
    {

        public static async Task execute(string credential, string url, string projectName, string username)
        {
            // Generate module header
            Console.WriteLine(Utilities.ArgUtils.GenerateHeader("addprojectadmin", credential, url, "", projectName, "", username));
            ...
            if (await Utilities.WebUtils.credsValid(credential, url))
            {
                ...
                try
                {
                    ...
                    // if user was added successfully, display message and list the members of the project admin group
                    if (userAdded)
                    {
