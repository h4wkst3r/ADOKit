...
namespace ADOKit.Modules.Recon
{
    class SearchProject
    {
        ...
        public static async Task execute(string credential, string url, string search)
        {
            ...
            Console.WriteLine(Utilities.ArgUtils.GenerateHeader("searchproject", credential, url, search, "", "", ""));
            ...
            if (await Utilities.WebUtils.credsValid(credential, url))
            {
                ...
                try
                {
                    ...
                    {
                        // if we have a match based on what user is searching for, display it
                        if (proj.projectName.ToLower().Contains(search.ToLower()))
                        {
